...
const ParticipationResult: NextPage = () => {
  ...
  const { mutate: handleArrived } = useArrivedMutation(date, {
    onSuccess: (data) => queryClient.invalidateQueries(VOTE_GET),
    onError: (err) => {
      toast({
        title: "오류",
  ...
  const participation = vote.participations.find(
    (p) => placeId === (p.place as IPlace)._id.toString()
  );

  const status = participation.status;
  const place = participation.place as IPlace;
  const confirmedAttendences = participation.attendences.filter(
    (att) => att.confirmed
  );
  ...
  const myAttendence = participation.attendences.find(
  ...
  const canShowAttendedButton = () => {
    ...
    const { start: markedStart, end: markedEnd } = myAttendence.time;
  ...
  const showConfirmButton =
    myAttendence && !myAttendence.confirmed && status === "waiting_confirm";
  ...
  return (
    <>
      <VStack>
              ...
              {confirmedUser.map((user, idx) => (
                <Box key={user.id} margin="0">
                  <ProfileImage
                    ...
                    top="0"
                    right={`${
                      -55 * ((confirmedUser.length + 1) / 2 - (idx + 1))
                    }px`}
                    key={user.uid}
        ...
        <AspectRatio ratio={1 / 1} width="90px">
          <Image
            src={place.image}
            alt={place.fullname}
            borderRadius="15px"
        ...
        {status !== "dismissed" && (
          <>
            ...
            <Container>
              <Heading as="h2" marginTop="15px" fontSize="lg">
                참여현황
              </Heading>
            ...
            {myAttendence?.confirmed && !myAttendence?.arrived && (
              <Button size="sm" colorScheme="red" onClick={onDismissAlertOpen}>
                불참
              </Button>
            )}
          </>
        ...
        {status === "dismissed" && (
          <>
            ...
            {participation.desc && <Text>{participation.desc}</Text>}
          </>
        )}
      </VStack>
      <Box
