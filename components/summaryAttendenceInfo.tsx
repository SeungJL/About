...
const SummaryAttendenceInfo: FC<{
  ...
  const attendence7Days = attendences?.filter(
    (a) => dayjs(a.date) >= oneWeekAgo
  );
  const cntVote7days = attendence7Days?.length;
  const cntOpen7days = attendence7Days?.filter(
    (a) => a.meetingTime !== ""
  )?.length;

  const attendence2Weeks = attendences?.filter(
    (a) => dayjs(a.date) >= twoWeeksAgo
  );
  const cntVote2Weeks = attendence2Weeks?.length;
  const cntOpen2Weeks = attendence2Weeks?.filter(
    (a) => a.meetingTime !== ""
  )?.length;
  ...
  const attendenceStatus = getAttendStatus(
    cntOpen7days,
    cntVote7days,
    cntOpen2Weeks,
    cntVote2Weeks,
    ...
    <HStack justifyContent="space-between" margin="5px 0">
      <Box key="1week" flex={1}>
        <HStack justifyContent="center" alignItems="center">
          <Text as="span" fontSize="sm" width="fit-content" marginRight="2px">
            7일
          </Text>
          <InfoPopOver
            content={"최근 7일간 참여횟수/투표횟수"}
            placement="top-start"
          />
        </HStack>
