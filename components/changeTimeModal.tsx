...
const ChangeTimeModal: FC<{
  ...
  const [start, setStart] = useState<Dayjs>(
    dateToDayjs(myParticipantTime.start)
  );
  const [end, setEnd] = useState<Dayjs>(dateToDayjs(myParticipantTime.end));

  const { mutate: handleChangeTime, isLoading } = useChangeTimeMutation(date, {
    ...
    <Modal size="md" onClose={onClose} isOpen={isOpen} isCentered>
      ...
      <ModalContent>
        ...
        <ModalFooter>
          ...
          <Button isLoading={isLoading} colorScheme="yellow" onClick={onSubmit}>
            변경
          </Button>
        </ModalFooter>
      </ModalContent>
