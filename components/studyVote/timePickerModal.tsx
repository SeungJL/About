...
const TimePickerModal: FC<{
  ...
  const onSubmit = async (
    values: { hour: string; minute: string },
    action: FormikHelpers<{ hour: string; minute: string }>
  ) => {
  ...
  const validateHour = (hour: string) => {
    if (!hour) return "시간을 입력해주세요";

    if (!hours.includes(parseInt(hour))) return "시간이 올바르지 않아요";

    return;
  };

  const validateMinute = (minute: string) => {
    if (!minute) return "분을 입력해주세요";

    if (!minutes.includes(minute)) return "분이 올바르지 않아요";
              ...
              <ModalBody>
                ...
                >
                  ...
                  <Field name="minute" validate={validateMinute}>
                    {({ field, form }) => (
                      <FormControl
                        isInvalid={form.errors.minute && form.touched.minute}
                        width="fit-content"
                      >
