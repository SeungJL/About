...
const isAlone = (
  participationTime: IParticipantTime,
  commonTime: boolean[]
) => {
  const { start, end } = participationTime;
  const startDayjs = dateToDayjs(start);
  ...
  return (
    myTime.map((t) => timeRange.indexOf(t)).filter((idx) => commonTime[idx])
      .length === 0
  );
};
...
const getOptimalTime = (times: string[]) => {
  const hourMinutes = times
    ...
    .sort(
      (a, b) => a.hour - b.hour + (a.hour === b.hour ? a.minute - b.minute : 0)
    );

  if (hourMinutes.length === 0) return "13:00";
  if (hourMinutes.length === 1)
    return `${hourMinutes[0].hour}:${String(hourMinutes[0].minute).padStart(
      2,
      "0"
    )}`;

  return `${hourMinutes[1].hour}:${String(hourMinutes[1].minute).padStart(
    2,
    "0"
  )}`;
};
