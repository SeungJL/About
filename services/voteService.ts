...
export const confirm = async (dateStr: string) => {
  ...
  const sources = vote.participations.filter(
    (participation: IParticipation) => {
      const attendenceCnt = participation.attendences.length;
      const unconfirmedCnt = participation.attendences.filter(
        (att) => !att.confirmed
      ).length;
  ...
  for (let i = 0; i < sources.length; i++) {
    ...
    if (targetParticipation1) {
      targetParticipation1.attendences = [
        ...targetParticipation1.attendences,
        attendence,
      ];
    ...
    const targetParticipation2 = vote.participations
      ...
      .find(
        (p) =>
          p.attendences.length < MAX_USER_PER_PLACE &&
          openable([...p.attendences.map((att) => att.time), attendence.time])
      );
  ...
  vote.participations.forEach((participation) => {
    ...
    if (openable(participationTimes)) {
      ...
      participation.desc =
        participation.attendences.length >= MIN_USER_FOR_STUDY
          ? `최소 ${MIN_USER_FOR_STUDY}명의 참여시간이 1시간 이상 겹치지 않아요`
          : `인원이 부족해요 (최소 인원: ${MIN_USER_FOR_STUDY})`;
    }
...
export const confirmWithValidating = async (dateStr) => {
  ...
  const voteEndTime = dayjsDate.subtract(1, "day").add(VOTE_END_HOUR, "hour");
